...
#define debugf(format, args...) \
	{if (__DEBUG__) {char __str[128]; sprintf (__str, format, args);USART1_Write ((const char*) __str);}}

#define debugln(__str)      {if (__DEBUG__) {USART1_WriteLine ((const char*) __str);}}
#define debug  (__str)      {if (__DEBUG__) {USART1_Write     ((const char*) __str);}}
